...
function renderListCategory(data) {
    let output = "";
    output += '<li class="cursor-pointer text-[#fbdb6d] capitalize">all</li>';
    ...
    $("#list-category li").each(function () {
        if ($(this).text() == currenMenuCategory) {
            $(this).addClass("text-[#fbdb6d]");
        } else {
            $(this).removeClass("text-[#fbdb6d]");
        }
    });

    $("#list-category li").click(function () {
        $("#list-category li").removeClass("text-[#fbdb6d]");
        $(this).addClass("text-[#fbdb6d]");
...
function renderNavPage() {
    for (let i = 0; i < pages.length; i++) {
        let liElement = $("<li>")
            ...
            .addClass(
                "group relative w-full rounded border-2 border-[#535353] px-1 py-0.5 text-center md:border-none md:border-0 md:p-0 nav-page "
            )
            .on("click", function () {
                setPage(pages[i]);
                setNavPage(pages[i]);
        ...
        let divElement = $("<div>").addClass(
            "absolute -bottom-1 h-1 bg-[#fbdb6d] duration-300 md:group-hover:w-full hidden md:block"
        );

        if (pages[i] === pages[0]) {
            liElement.addClass(
                "bg-[#535353] md:bg-white text-white md:text-black"
            );
...
function setNavPage(page) {
    if ($(window).innerWidth() <= 768) {
        $(".nav-page").each(function () {
            $(this)
                .removeClass("bg-[#535353] text-white")
                .addClass("bg-white text-black");
            if ($(this).data("page") === page) {
                $(this)
                    .addClass("bg-[#535353] text-white")
...
function createProductElement(item) {
    ...
    let cartButtonElement = $("<button>")
        .addClass(
            "px-1 border-2 border-[#535353] rounded-full text-xs hover:text-white md:hover:bg-[#535353] duration-200 cursor-pointer btn-add-to-cart"
        )
        .text("add to cart")
...
function changeGroupMenu(current) {
    ...
    $(".btn-indicator").each(function () {
        $(this).removeClass("bg-[#535353]").addClass("bg-[#a78b71]");
        if ($(this).data("indicator-no") === current) {
            $(this).removeClass("bg-[#a78b71]").addClass("bg-[#535353]");
        }
    });
}
...
function deleteProductInCart(idProduct, type) {
    ...
    if (type === "direct") {
        ...
        swal.fire({
            ...
            reverseButtons: true,
            customClass: {
                confirmButton: 'bg-[#ff78c4] text-white',
                cancelButton: 'bg-[#535353] text-white',
            },
        }).then((result) => {
...
function checkOrder() {
    ...
    if (
        ...
        $("#btn-checkout")
            ...
            .addClass("bg-[#ff78c4] md:hover:bg-[#ff78c4]")
            .off("click")
            .on("click", function () {
                makeOrder();
            });
        ...
        $("#btn-checkout")
            .removeClass("bg-[#ff78c4] md:hover:bg-[#ff78c4]")
