...
function renderNavPage() {
    for (let i = 0; i < pages.length; i++) {
        let liElement = $("<li>")
            ...
            .addClass(
                "group relative w-full rounded border-2 border-[#535353] px-1 py-0.5 text-center md:border-none md:p-0 nav-page "
            )
            .on("click", function () {
                setPage(pages[i]);
                setNavPage(pages[i]);
...
function getJson(type, callback) {
    switch (type) {
        case "product":
            $.get("../json/produk.json").done((data) => {
                callback(data);
            });
            break;
        default:
            console.log(`file json ${type} tidak ditemukan`);
    }
}
...
function renderProduct(product) {
    product.forEach((item) => {
        if (item.status_best_seller === true) {
...
function getProductByCategory(category) {
    ...
    getJson("product", function (data) {
        ...
        if (currentCategory !== "all") {
            product = data.filter((item) => item.katgori === currentCategory);
        }
...
function checkOrder() {
    ...
    if (
        ...
        $("#btn-checkout")
            ...
            .addClass("bg-[#9c4a1a] md:hover:bg-[#535353]")
            .off("click")
            .on("click", function () {
                makeOrder();
            });
        ...
        $("#btn-checkout")
            .removeClass("bg-[#9c4a1a] md:hover:bg-[#535353]")
